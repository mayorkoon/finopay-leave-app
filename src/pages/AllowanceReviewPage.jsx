...
import { processMdAllowanceDecision, getLeaveByAllowanceToken } from "../services/leaveService";
...
function InfoRow({ label, value }) {
  return (
    <div style={{
      ...
      <span style={{ fontSize: 11, color: "#94a3b8", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", flexShrink: 0, fontFamily: "'DM Sans', sans-serif" }}>
        {label}
      </span>
      <span style={{ fontSize: 13, color: "#1e293b", fontWeight: 500, textAlign: "right", fontFamily: "'DM Sans', sans-serif" }}>
        {value || "—"}
      </span>
    </div>
  );
...
export default function AllowanceReviewPage() {
  ...
  useEffect(() => {
    ...
    getLeaveByAllowanceToken(token)
      .then((data) => {
        ...
        if (data.allowanceStatus !== "pending_md") {
          setStatus("error");
          setErrorMsg("This allowance request has already been actioned.");
          return;
        }
        if (data.allowanceTokenExpiry && new Date() > new Date(data.allowanceTokenExpiry)) {
          setStatus("error");
          setErrorMsg("This approval link has expired (72 hour limit).");
          return;
        }
        setLeave(data);
        setStatus("ready");
      ...
      .catch(() => {
        setStatus("error");
        setErrorMsg("Failed to load the request. Please try again.");
      });
  }, [token]);
  ...
  const cardStyle = {
    ...
    padding: "40px 40px", maxWidth: 520, width: "100%",
    ...
    return (
      <div style={pageStyle}>
        <div style={{ ...cardStyle, textAlign: "center" }}>
          ...
          <p style={{ color: "#64748b", fontSize: 15 }}>Loading request details...</p>
        </div>
      </div>
    );
  }
      ...
      <div style={pageStyle}>
        <div style={{ ...cardStyle, textAlign: "center" }}>
          ...
          <h2 style={{ fontSize: 22, fontWeight: 800, color: "#1e293b", marginBottom: 8 }}>
            Allowance {result.approved ? "Approved" : "Rejected"}
          </h2>
          <p style={{ color: "#64748b", fontSize: 14, lineHeight: 1.6 }}>
            You have {result.approved ? "approved" : "rejected"} the leave allowance
            for <strong style={{ color: "#1e293b" }}>{result.staffName}</strong>.
      ...
      <div style={cardStyle}>
        ...
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <div style={{
            ...
            <span style={{ fontSize: 12, fontWeight: 700, color: "#c2410c", textTransform: "uppercase", letterSpacing: "0.05em" }}>
              Leave Allowance Approval
            </span>
          </div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "#1e293b", marginBottom: 6, letterSpacing: "-0.02em" }}>
            Allowance Request
          </h1>
          <p style={{ color: "#64748b", fontSize: 13, lineHeight: 1.6 }}>
            Please review the details below and approve or reject the allowance request.
            The staff member's leave is already approved — this decision only affects the allowance.
          </p>
        </div>
        ...
        <div style={{ display: "flex", gap: 12 }}>
          <button
            ...
            style={{
              ...
              background: status === "processing" ? "#94a3b8" : "linear-gradient(135deg, #059669, #10b981)",
              color: "#fff", fontSize: 14, fontWeight: 700,
              cursor: status === "processing" ? "not-allowed" : "pointer",
              fontFamily: "'DM Sans', sans-serif",
              boxShadow: status === "processing" ? "none" : "0 4px 14px rgba(16,185,129,0.35)",
            }}
          >
